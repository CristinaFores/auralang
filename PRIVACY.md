...
1. **Transcription**: audio is transcribed locally, on your own device, using a Whisper model (`@huggingface/transformers`, ONNX Runtime WASM). The model weights (~75MB) are downloaded once from Hugging Face (huggingface.co) and cached locally. Audio never leaves your device for transcription.
